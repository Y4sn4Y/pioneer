...
bool Bhv_BasicMove::press(rcsc::PlayerAgent * agent)
{
    ...
    const PlayerObject * opp_ball = wm.getOpponentNearestToBall(5,true);
    Vector2D oppon_ball =opp_ball->pos();
    Vector2D op_pos = op->pos();
    const int self_min = wm.interceptTable()->selfReachCycle();
    const int mate_min = wm.interceptTable()->teammateReachCycle();
    const int opp_min = wm.interceptTable()->opponentReachCycle();
    double dist_tm = wm.getDistTeammateNearestTo(op_pos,5);
    if( dist_op>20 || self_min<opp_min || mate_min<opp_min || dist_tm<3 || dist_self_ball>11)
    {
        return false;
    }
    else if(dist_op-10 < oppon_ball)
    {
        Body_Intercept().execute(agent);
        return true;
    }
...
rcsc::Vector2D Bhv_BasicMove::new_position(const rcsc::WorldModel& wm, int unum)
{
    ...
    int teammae_reach_ball = wm.getDistTeammateNearestTo(ball_pos,5);
    int opp_reach_ball = wm.getDistOpponentNearestToBall(5 , true);
    int pos[17][7]=
    {
        {0,0,0,0,0,0}, //0
        {0,0,18,20,1,6},//1
    ...
    if (pos[number][unum] == 4 || pos[number][unum] == 5 || pos[number][unum] == 20 || pos[number][unum] == 21 )
    {
        if (teammae_reach_ball-3 > opp_reach_ball)
        {
            Body_TurnToBall().ee5gxecute(agent);
        }
    }
